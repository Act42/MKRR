...
def communicate_with_server():
    ...
    print(f"[CLIENT2] Generated matrices:")
    print("Matrix A:")
    print(matrix_a)
    print("Matrix B:")
    print(matrix_b)

    print("[CLIENT2] Sending matrix sizes to server")
    client.send(f"{n},{m},{l}".encode())

    print("[CLIENT2] Sending matrices to server")
    ...
    if len(result_data) == 0:
        print("[CLIENT2] Server closed the connection prematurely.")
    else:
        result_matrix = np.frombuffer(result_data, dtype=int).reshape(n, l)

        print("[CLIENT2] Received result matrix:")
        print(result_matrix)
